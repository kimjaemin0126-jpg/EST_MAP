...
import { requestScenario } from '../services/compare'
import {
  fetchJsonCached,
  mergeMarketContext,
  mergeProcessedData,
...
function TargetSelector({ side, value, options, details, onChange, disabled }) {
  ...
  return (
    <article className={`target-selector ${side}`}>
      <div className="target-heading">
        <span>{label}</span>
        <strong>{details.dong?.name || '지역 선택'}</strong>
      </div>
      <div className="region-search-field">
        <label htmlFor={inputId}>행정동 검색·선택</label>
        <input
          id={inputId}
          ref={inputRef}
          type="search"
...
export default function ComparisonTestPage() {
    ...
    Promise.all([
      Promise.all(COMPARISON_QUARTERS.map((quarter) => (
        fetchJsonCached(
          ...
          `${quarter} EST_MAP 점포 데이터`,
        )
      ))),
      Promise.all(COMPARISON_QUARTERS.map((quarter) => (
        fetchJsonCached(
          DATA_PATHS.marketContext(quarter),
          `${quarter} EST_MAP 상권 맥락 데이터`,
        )
      ))),
    ])
      ...
      .catch(() => {
        if (active) setStaticError('EST_MAP 통계 데이터를 불러오지 못했습니다.')
      })
      .finally(() => {
  ...
  async function runComparison() {
    ...
    try {
      ...
      setAnalysis(null)
      setApiError(error.message || 'OpenSafe AI 비교 요청에 실패했습니다.')
    } finally {
      setApiLoading(false)
    }
  }
  ...
  return (
    <div className="comparison-page">
      <header className="comparison-header">
        <div>
          <span className="comparison-kicker">EST_MAP × OpenSafe AI</span>
          <h1>두 지역 상권 통계 비교</h1>
        </div>
        <a href="/">지도 화면으로 이동</a>
      </header>
      ...
      <main className="comparison-content compact">
        ...
        {apiError && <div className="comparison-alert error">{apiError} OpenSafe AI 서버가 실행 중인지 확인하세요.</div>}
          ...
          <section className="comparison-section glance-section" aria-labelledby="glance-title">
            <div className="comparison-section-heading">
              <div>
                ...
                <p>
                  EST_MAP {COMPARISON_YEAR}년 연간 · AI {quarterLabel(leftAi.meta.latest_quarter)}
