...
import { FEATURE_ROUTES } from '../config/featureRoutes'
    ...
    <Marker position={center} icon={anchorIcon} interactive={false} keyboard={false}>
      ...
      >
        <span className="landing-selection-impact-content">
          ...
          <a href={`/map?region=${selectedCode}`}>
            상세 보기 <ArrowIcon />
          </a>
        </span>
      </Tooltip>
...
export default function LandingPage() {
  ...
  const handleSubmit = (event) => {
    ...
    window.location.assign(`/map?region=${targetCode}`)
  }
    ...
    <div className="landing-root">
      <header className="landing-header">
        ...
        <nav className="landing-nav" aria-label="주요 메뉴">
          ...
          {FEATURE_ROUTES.startupScenario ? (
            <a href={FEATURE_ROUTES.startupScenario}>창업 시나리오</a>
          ) : (
            <button type="button" className="landing-nav-pending">창업 시나리오</button>
          )}
        </nav>
      </header>
      ...
      <main>
        <section className="landing-hero">
          <div className="landing-copy">
            ...
            <a className="landing-map-link entrance entrance-5" href="/map">
