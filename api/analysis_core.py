...
def _code(value: Any) -> str:
    """Normalize codes coming from JSON/JS without changing their digits."""
    if value is None:
        return ""
...
def find_record(
    ...
    dong_code = _code(dong_code)
    industry_code = _code(industry_code)
    dong_name = str(dong_name or "").strip()
    industry_name = str(industry_name or "").strip()

    industry_rows = [
        row for row in records
        if _code(row.get("ic")) == industry_code
    ]
    record = next(
        (row for row in industry_rows if _code(row.get("dc")) == dong_code),
        None,
    )

    # The map dataset and analysis dataset can use different administrative
    # code vintages. If the exact code pair misses, resolve the same selection
    # by the human-readable dong/industry names already supplied by the UI.
    if not record and dong_name and industry_name:
        record = next(
            (
                row for row in records
                if str(row.get("d", "")).strip() == dong_name
                and str(row.get("i", "")).strip() == industry_name
            ),
            None,
        )
        if record:
            industry_code = _code(record.get("ic"))
            industry_rows = [
                row for row in records
                if _code(row.get("ic")) == industry_code
            ]
    ...
    if not record:
        raise ValueError(
            "선택한 행정동·업종 조합의 분석 데이터가 없습니다. "
            f"(행정동 코드: {dong_code or '-'}, 업종 코드: {industry_code or '-'})"
        )
...
def scenario_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    dong_code = _code(payload.get("dong_code", ""))
    industry_code = _code(payload.get("industry_code", ""))
    dong_name = str(payload.get("dong_name", "") or "").strip()
    industry_name = str(payload.get("industry_name", "") or "").strip()
    record, industry_rows = find_record(
        dong_code,
        industry_code,
        dong_name=dong_name,
