...
def scenario_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "meta": {
            "latest_quarter": data_meta.get("latest_quarter"),
            "forecast_quarter": data_meta.get("ai_model", {}).get("forecast_quarter"),
            "model_name": data_meta.get("ai_model", {}).get("model_name"),
        },
        "market": {
            "dong": record["d"],
            "dong_code": record["dc"],
            "industry": record["i"],
            "industry_code": record["ic"],
